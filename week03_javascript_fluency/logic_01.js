...
function multiply() {
    var number_count = document.getElementById("count").value;
    var multiplier = document.getElementById("multiplier").value;
    var loop_table = document.getElementById("loop_table");
    //clear old table cells
    while (loop_table.rows.length > 1) {
        loop_table.deleteRow(1);
    }
    for (var count = 1; count <= number_count; count++) {
            var row_at_count = loop_table.insertRow(count);
            var cell_number_at_count = row_at_count.insertCell(0);
            cell_number_at_count.innerHTML = count;
            var cell_number_multiplied = row_at_count.insertCell(1);
...
function save_item_to_array() {
    ...
    for (item of items_array) {
        var row_at_index = array_table.insertRow(index + 1);
        var cell_index = row_at_index.insertCell(0);
        cell_index.innerHTML = index;
        var cell_item_name = row_at_index.insertCell(1);
...
function save_to_associative_array() {
    var friend_name_input = document.getElementById("friend_name_input");
    var friends_money_input = document.getElementById("friend_money_input");
    var friends_table = document.getElementById("associative_array_table");
    ...
    for (friend of friends_array) {
        var row_at_index = friends_table.insertRow(index + 1);
        var cell_name = row_at_index.insertCell(0);
        cell_name.innerHTML = friend.name;
        var cell_money = row_at_index.insertCell(1);
